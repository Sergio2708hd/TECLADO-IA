...
def predecir_palabra(texto, modelo):
    ...
    return sorted(opciones)[:3]  # Máximo 3 sugerencias

cap = cv2.VideoCapture(0)
cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)   # Configurar ancho de la cámara
cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)   # Configurar alto de la cámara
...
while True:
    ...
    for fila_idx, fila in enumerate(filas):
        ...
        for col_idx, tecla in enumerate(fila):
            ...
            # Color azul cielo: (135, 206, 250) en BGR sería (250, 206, 135)
            cv2.rectangle(frame, (x, y), (x + tecla_size, y + tecla_size), (250, 206, 135), -1)
            texto_tecla = " " if tecla == "ESPACIO" else tecla
            cv2.putText(frame, texto_tecla, (x + 10, y + 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 2)
